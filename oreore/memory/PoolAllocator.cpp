...
namespace OreOreLib
{
	...
	void* PoolAllocator::Allocate( size_t alignment )
	{
		...
		{
			// Shift m_DirtyFront if page is the first dirty element.
			if( page == m_DirtyFront )	m_DirtyFront = page->next;

			// Disconnect page link
			page->Disconnect();

			// Connect after m_UsedupFront
			if( m_UsedupFront )	page->ConnectAfter( m_UsedupFront );
			else				m_UsedupFront = page;
		}
	...
	bool PoolAllocator::Free( void*& ptr, Page* page )
	{
		...
		if( stateBefore==Usedup && stateAfter==Dirty )
		{
			...
			// Shift m_UsedupFront if page is the first usedup element.
			...
			// Connect after m_UsedupFront
			if( m_DirtyFront )	page->ConnectAfter( m_DirtyFront );
			else				m_DirtyFront = page;
		...
		else if( stateBefore==Dirty && stateAfter==Clean )
		{
			...
			// Connect after m_CleanFront
			if( m_CleanFront )	page->ConnectAfter( m_CleanFront );
			else				m_CleanFront = page;
	...
	void PoolAllocator::AllocatePages( uint32 numPages )
	{
		...
		for( uint32 i=0; i<numPages; ++i, ++m_CurrentCommitedPageCount )
		{
			...
			//========================= Page deployment ==========================//
			//if( m_CleanFront )	newPage->ConnectBefore( m_CleanFront );
			//m_CleanFront = newPage;

			if( m_CleanFront )	newPage->ConnectAfter( m_CleanFront );
			else m_CleanFront = newPage;
	...
	{		
		...
		for( Page* p=m_UsedupFront; p!=nullptr; p=p->next )
			if( mem == (void*)( (size_t)p & RegionTag::AlignmentMask) )
				return true;

		for( Page* p=m_DirtyFront; p!=nullptr; p=p->next )
			if( mem == (void*)( (size_t)p & RegionTag::AlignmentMask) )
				return true;
